    ...
    <pattern abstract="true" id="idref-any">
        <rule context="$element[@$idref-attr-name]">
            <assert test="some $elem in $id-set satisfies $elem/@id eq current()/@$idref-attr-name"
                >The <value-of select="'$idref-attr-name'"/> attribute must refer to an element in the same document (the ID '<value-of 
    ...
    <pattern abstract="true" id="idrefs-any">
        <rule context="$element[@$idrefs-attr-name]">            
            <assert test="every $idref in tokenize(@$idrefs-attr-name,'\s+') satisfies (some $elem in $id-set satisfies ($elem/@id eq $idref))"
                >The <value-of select="'$idrefs-attr-name'"/> attribute must refer to elements in the same document (target ID missing)</assert>
        </rule>
    </pattern>

    <pattern abstract="true" id="idref-named">
        <rule context="$element[@$idref-attr-name]">            
            <assert test="//$target-name[@id = current()/@$idref-attr-name]">The <value-of
                    select="'$idref-attr-name'"/> attribute does not refer to an allowed target element (expecting: <value-of 
